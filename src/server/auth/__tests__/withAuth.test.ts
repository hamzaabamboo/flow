...
describe('withAuth Elysia middleware', () => {
  ...
  it('should fail with expired API token', async () => {
    ...
    const mockTokenRecord = { id: 't1', token: 'mock-hash-sha256-hex-expired-token', userId: 'u1', expiresAt: expiredDate };
  ...
  it('should validate with HamAuth if not an API token', async () => {
    ...
    (validateHamAuthTokenCached as any).mockResolvedValue({ sub: 'ext-1', email: 'ham@example.com' });
    
    // 3. Find or create user
    (db as any).select.mockReturnValueOnce(createMockQueryBuilder([{ id: 'u2', email: 'ham@example.com' }]));
