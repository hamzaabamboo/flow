  ...
  return render(
    <QueryClientProvider client={queryClient}>
      <SpaceContext.Provider value={mockSpaceContext as any}>
        <ToasterContext.Provider value={mockToasterContext}>
          {ui}
...
describe('SettingsPage', () => {
  beforeEach(() => {
    ...
    (api.api.notes.collections.get as vi.Mock).mockResolvedValue({ data: { data: [{ id: 'coll-1', name: 'Collection 1' }] }, error: null });
    (api.api.calendar['feed-url'].get as vi.Mock).mockResolvedValue({ data: { url: 'http://cal.feed' }, error: null });
  ...
  it('should handle external calendar addition', async () => {
    ...
    await user.type(within(addDialog).getByPlaceholderText(/https:\/\/calendar\.google\.com/i), 'http://test.com/cal.ics');
    
    await user.click(within(addDialog).getByRole('button', { name: 'Add Calendar' }));

    expect(mockExternalCalendarsPost).toHaveBeenCalledWith(expect.objectContaining({
      name: 'My Calendar',
  ...
  it('should save Outline settings', async () => {
    ...
    expect(api.api.settings.patch).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('should handle external calendar editing', async () => {
    const user = userEvent.setup();
    const mockCal = { id: 'cal-1', name: 'Work Cal', icalUrl: 'http://url', enabled: true, space: 'work', color: 'blue' };
    ...
    expect(mockExternalCalendarsPatch).toHaveBeenCalledWith(expect.objectContaining({
      name: 'New Name'
    }));
  });
