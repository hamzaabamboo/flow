...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  loadTokens: async () => {
    try {
      ...
      // If token is expired and we have a refresh token, refresh in background
      if (isExpired && refreshToken) {
        console.log('[AuthStore] Token expired, will refresh in background');
        // Set auth state immediately to unblock UI, then refresh in background
        ...
        refreshAccessToken()
          .then((refreshed) => {
            if (!refreshed) {
              console.log('[AuthStore] Background token refresh failed, clearing auth state');
              set({
                accessToken: null,
                refreshToken: null,
                isAuthenticated: false,
          ...
          .catch((error) => {
            console.error('[AuthStore] Background token refresh error:', error);
          });
        return;
      }
  ...
  checkAndRefreshToken: async () => {
    ...
    if (!tokenExpiry) {
      console.log('[AuthStore] No token expiry found, attempting refresh anyway');
      const { refreshAccessToken } = get();
    ...
    console.log('[AuthStore] Token still valid, no refresh needed');
    return true;
  },
...
export const getStoredAccessToken = async (): Promise<string | null> => {
  try {
    ...
    console.log(
      '[getStoredAccessToken] Token retrieved:',
